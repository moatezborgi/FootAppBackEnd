...
public class JwtRefreshTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            log.info("token is :{}",request.getHeader(HttpHeaders.AUTHORIZATION).substring(7));
            ...
            if (!userName.isEmpty() && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                var refreshTokenOptional = userRefreshTokenRepository.findByRefreshToken(jwtRefreshToken.getTokenValue());
                var isRefreshTokenValidInDatabase = refreshTokenOptional.isRevoked();


                UserDetails userDetails = userManagerInfo.loadUserByUsername(userName);
